...
from src.hedge_ratio import construct_spread
from src.hedge_ratio import get_OLS_hedge_ratio
from src.cointegration import EngleGrangerPortfolio
from src.cointegration import get_half_life_of_mean_reversion, get_hurst_exponent

class CointegrationSelector:
    def __init__(self, universe: pd.DataFrame = None, baskets: list = None):
        """
        Constructor.
        ...
        if universe is not None and baskets is not None:
            self.set_prices(universe, baskets)
    ...
    def set_prices(self, prices_df: pd.DataFrame, baskets_to_filter: list):
        ...
        self.baskets = baskets_to_filter
    ...
    def construct_spreads(self, hedge_ratio_calculation: str) -> dict:
        """
        For `self.baskets` construct spreads and log hedge ratio
    ...
    def select_spreads(self, hedge_ratio_method: str = 'OLS',
                       adf_cutoff_threshold: float = 0.95,
                       hurst_exp_threshold: float = 0.5,
                       min_crossover_threshold: int = 12,
                       min_half_life: float = 365) -> list:
        ...
        :param hedge_ratio_method: (str) Defines how hedge ratio is calculated. Can be either 'OLS',
            'TLS' (Total Least Squares), 'min_half_life', 'min_adf', 'johansen', 'box_tiao'.
        :param adf_cutoff_threshold: (float) ADF test threshold used to define if the spread is cointegrated.
            Can be 0.99, 0.95 or 0.9.
        :param hurst_exp_threshold: (float) Max Hurst threshold value.
        ...
        self.spreads_dict = self.construct_spreads(hedge_ratio_method)
