...
class MPICopulaTradingRule:
    ...
    def _exit_trigger_mpi(self, pre_flag: pd.Series, raw_cur_flag: pd.Series, open_based_on: list,
        ...
        raw_cur_flag_1 = raw_cur_flag.iloc[0]  # Current raw flag1 value
        raw_cur_flag_2 = raw_cur_flag.iloc[1]  # Current raw flag2 value
    ...
    @staticmethod
    def plot_positions_n_flags(positions: pd.Series, flags: pd.DataFrame, title: str = ""):

        fig, axs = plt.subplots(2, 1, gridspec_kw={'height_ratios': [1, 0.4]}, figsize=(10, 6), dpi = 150)
        # fig, axes = plt.subplots()
        fig.suptitle('Flags and Corresponding Positions')

        # Plotting flags for Student-t copula
        axs[0].plot(flags.iloc[:, 0], label = flags.columns[0])
        axs[0].plot(flags.iloc[:, 1], label= flags.columns[1])
        axs[0].title.set_text(f'Flags {title}')
        axs[0].legend()
        axs[0].grid()
        # axs[0].xaxis.set_major_locator(mdates.MonthLocator(bymonthday=1))
        ...
        axs[1].plot(positions, label='Positions', color='brown')
        axs[1].title.set_text(f'Positions {title}')
        axs[1].grid()
