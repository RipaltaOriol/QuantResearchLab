...
def construct_spread(price_data: pd.DataFrame, hedge_ratios = pd.Series, dependent_var: str = None) -> pd.Series:
    """
    Construct spread from `price_data` and `hedge_ratios`. If a user sets `dependent_var` it means that a
    spread will be:
    ...
    hedge_ratios.index = price_data.columns
    weighted_prices = price_data * hedge_ratios  # price * hedge

    if dependent_var is not None:
        print("This should not be triggered")
        non_dependent_variables = [x for x in weighted_prices.columns if x != dependent_var]
        return weighted_prices[dependent_var] - weighted_prices[non_dependent_variables].sum(axis=1)
